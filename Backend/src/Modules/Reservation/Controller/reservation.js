...
//m7tag 23ml check en mafeesh fulldate == fulldate 3shan hyb2a etneen 7agzeen fe nafs elwa2t bs lw doctor tany tmam
    ...
    client.query(query,(err,result)=>{
      ...
      //  console.log(result.rows[0].slotid)
        ...
        const query6 = `SELECT * from reservations WHERE keys = ${keys}`
        ...
        let id=1
        const query1 = `SELECT * from reservations`
...
const query5 = `SELECT * FROM slots WHERE key = ${keys}`

client.query(query5,(err,result)=>{

console.log(result.rows[0].doctorname)
            const query3 = `INSERT INTO reservations(id,slotid,patientid,doctorid,doctorname,fulldate,status,keys) VALUES(${id},${result.rows[0].id},${userId},${result.rows[0].doctorid},'${result.rows[0].doctorname}','${result.rows[0].fulldate}','${status}',${keys})`
            console.log(result.rows[0].id,userId,result.rows[0].doctorid,result.rows[0].doctorname,result.rows[0].fulldate,status)
...
const updateReservation = (req,res,next)=>{
    ...
    const query1 = `SELECT * from reservations WHERE id = ${id}`
    client.query(query1,(err,result)=>{
        if(err)
        {
            return res.json({message: "err",err,errMessage:err.message,stack:err.stack})
...
//})
        ...
        const query2 = `SELECT * from reservations WHERE keys = ${keys}`
        client.query(query2,(err,result)=>{
            if(err)
            {
                return res.json({message: "err",err,errMessage:err.message,stack:err.stack})
...
// 3ayz 23ml check en lw 7ad 3amal update eno yd5l keys 7ad wa5dha ablo en ttl3lo error
        ...
        const query3 = `UPDATE reservations
SET keys=${keys}, doctorid = ${result.rows[0].doctorid}, slotid = '${result.rows[0].id}', status = '${status}'
